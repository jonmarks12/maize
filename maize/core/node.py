...
class Node(Component, Runnable, register=False):
    ...
    fail_ok: Flag = Flag(default=False)
    """Whether the node is allowed to fail without crashing the full workflow"""
    ...
    def execute(self) -> None:  # pragma: no cover
        ...
        try:
            ...
            if self.status == Status.FAILED and not self.fail_ok.value:
                self.signal.set()
                if tbe is not None:
    ...
    def _prepare_iteration(self) -> None:
        """Prepares an iteration by cleaning up temporary directories"""
    ...
    def _attempt_loop(self) -> traceback.TracebackException | None:
        ...
        """
        # Skip execution if node is set to inactive
        if not self.active.value:
            self.logger.info("Node inactive, stopping...")
            self.status = Status.STOPPED
            return None

        # Skip execution if node parameters are not set: In some cases we might have
        # branches in our workflow dedicated to providing a potentially optional input
        # for a downstream node. This means we don't want to force the user to set this
        # value, so we want to be able to override the originating parameter to be optional.
        # This should then cause the loading node to not run at all, which will cause this
        # branch to shutdown. These conditions here check that this is the case, i.e. if all
        # parameters are unset and optional, and none of the inputs are connected,
        ...
        # Error in run()
            ...
            self.logger.error("Failed due to exception", exc_info=err)
            ...
            # and (maybe) try again
            return traceback.TracebackException.from_exception(err)
