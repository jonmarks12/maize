...
ProcessType = Union["Process", "SpawnProcess", "ForkProcess", "ForkServerProcess"]
...
class RunPool:
    ...
    def __enter__(self) -> "RunPool":
        ...
        # Extra safe auto-cleanup on exit
        atexit.register(cleanup_processes, items=self.items, procs=self._procs, wait_time=self.wait_time)
        return self
    ...
    def __exit__(self, *_: Any) -> None:
        ...
        for hand in log.handlers:
            hand.flush()
        cleanup_processes(self.items, self._procs, wait_time=self.wait_time)
