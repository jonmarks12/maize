...
from maize.utilities.references import Reference
...
def expose(factory: Callable[..., "Workflow"]) -> Callable[[], None]:
    ...
    def wrapped() -> None:
        ...
        parser.description = Workflow.get_workflow_summary(name=name)
...
class Workflow(Graph, register=False):
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any], check: bool = True, **kwargs: Any) -> "Workflow":
        """
    ...
    def update_settings_with_args(self, args: argparse.Namespace) -> None:
        ...
        if args.config:
            ...
            self.logger.debug("Using config at %s", Path(args.config).absolute())
            self.config.update(args.config)
        if args.quiet:
            self.logger.debug("Logging at warning level")
            self._set_global_attribute("level", logging.WARNING)
            self.logger.setLevel(logging.WARNING)
        if args.debug:
            self.logger.debug("Logging at debug level")
            self._set_global_attribute("level", logging.DEBUG)
            self.logger.setLevel(logging.DEBUG)
        if args.log:
            self.logger.debug("Will log to %s", Path(args.log).absolute())
            self._set_global_attribute("logfile", args.log)
        if args.keep or args.debug:
            self.logger.debug("Keeping all generated temporary files")
            self._set_global_attribute("cleanup_temp", False)
        if args.parameters:
            self.logger.debug("Reading additional parameters from %s", args.parameters)
            data = read_input(args.parameters)
            self.update_parameters(**data)
        if args.scratch:
            self.logger.debug("Using scratch at %s", Path(args.scratch).absolute())
            self._set_global_attribute("scratch", Path(args.scratch))

    def find_io(self) -> dict[str, Any]:
        """
        Finds all inputs, outputs, and parameters for the workflow.

        Returns
        -------
        dict[str, Any]
            Dictionary of interfaces and their set values and types
        ...
        for name, para in self.all_parameters.items():
            data[name] = {
                "name": name,
                "kind": para.__class__.__name__.lower(),
                "type": str(para.datatype),
                "value": para.value if para.is_set else None,
                "optional": para.optional,
                "is_file": is_path_type(para.datatype),
            }
        return data
    ...
    def execute(self) -> None:
        ...
        if refs := self.collect_references():
            msg = "Please cite the following:\n\n"
        ...
        def _telemetry(error: TracebackException | None = None) -> None:
            if (url := self.config.telemetry_url) is not None:
                self.logger.info("Sending workflow metadata to %s to help improve maize", url)
                try:
                    send_telemetry(
                        url=url,
                        workflow=self,
                        ...
                        error=error,
                        key=self.config.telemetry_key,
                    )
                except Exception as err:
                    self.logger.warning("Failed to send telemetry data: %s", err, exc_info=err)
