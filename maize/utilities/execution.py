...
def _log_command_output(stdout: bytes | None, stderr: bytes | None, name: str | None = None) -> str:
    """Write any command output to the log."""
    msg = f"Command output for {name}:\n" if name is not None else "Command output:\n"
...
class GPU:
    ...
    @classmethod
    def from_system(cls, verbose: bool = False) -> list[Self]:
        """Provides information on GPU capabilities"""
        cmd = CommandRunner(raise_on_failure=False)
        if verbose:
            cmd.run_only("nvidia-smi", verbose=verbose)
        ...
        for gpu in tree.findall("gpu"):
            ...
            devices.append(
                cls(
                    available=True,
                    free=gpu_ok,
                    free_with_mps=mps_only,
                    mode=GPUMode.EXCLUSIVE_PROCESS,
                )
            )
        return devices
...
def check_returncode(
    ...
    if result.returncode == 127:
        ...
        logger.warning(_log_command_output(result.stdout, result.stderr, name=result.args[0]))
        if raise_on_failure:
...
class RunningProcess(ProcessBase):
    ...
    def wait(self) -> subprocess.CompletedProcess[bytes]:
        ...
        if self.verbose:
            self.logger.debug(
                _log_command_output(result.stdout, result.stderr, name=self.job.native_id)
            )
...
class CommandRunnerPSIJ(ProcessBase):
        ...
        def _callback(job: Job, status: JobStatus) -> None:
            if status.final:
                ...
                if job.status.exit_code is not None and job.status.exit_code > 0:
                    ...
                    self.logger.warning(
                        _log_command_output(
                            stdout=res.stdout, stderr=res.stderr, name=job.native_id
                        )
                    )
                count.dec()
    ...
    def validate(self, result: subprocess.CompletedProcess[bytes]) -> None:
        ...
        for validator in self.validators:
            if not validator(result):
                ...
                self.logger.warning(
                    _log_command_output(result.stdout, result.stderr, name=result.args[0])
                )
    ...
    def run_only(
        ...
        self.check_returncode(result, raise_on_failure=self.raise_on_failure)
        if verbose:
            self.logger.debug(_log_command_output(result.stdout, result.stderr, name=job.native_id))
    ...
    def run_parallel(
        ...
        if config is not None:
            n_jobs = max(self.config.max_jobs, n_jobs)
        ...
        for job in jobs:
            ...
            if verbose:
                self.logger.debug(
                    _log_command_output(result.stdout, result.stderr, name=job.native_id)
                )
    ...
    def _attempt_submission(self, job: Job) -> Job:
        ...
        for i in range(self.max_retries):
            try:
                self._executor.submit(job)
            except (SubmitException, BlockingIOError) as err:
                self.logger.warning(
