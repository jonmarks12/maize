...
import string
import sys
from types import UnionType, FrameType
from typing import (
    TYPE_CHECKING,
    AnyStr,
    Literal,
...
def timeout(timeout: int) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
    """
...
def typecheck(value: Any, datatype: Any) -> bool:
    ...
    # Special check for datatypes that are considered as subtypes from a
    # Maize perspective, e.g. int can be considered a subtype of float
    if datatype in EQUIVALENT_TYPES:
        return isinstance(value, datatype | EQUIVALENT_TYPES[datatype])
...
class PathEncoder(json.JSONEncoder):  # pragma: no cover
...
def send_telemetry(
    ...
    error: TracebackException | None,
    key: str | None = None,
) -> None:
    """
    Tries to send telemetry data to Maize HQ, if available.

    Will attempt sending the workflow itself, its name, the responsible username, the host on
    which it is run, the submission and completion times, and any errors if they occurred.
    ...
    error
        An exception with traceback if one occurred
    key
    ...
    ------
    HTTPError
        If something went wrong with the telemetry request
    ...
    data = {
        ...
        "submitted": submission_time.isoformat(),
    ...
    # We play it safe and send incomplete data if we can't fully serialize everything
    try:
        data["workflow"] = json.dumps(workflow.to_dict(), cls=PathEncoder)
    except TypeError:
        data["workflow"] = "{}"

    # This allows us to get the full formatted traceback
    if error is not None:
        data["error"] = "\n".join(error.format())
        data["error_type"] = error.exc_type.__name__

    with requests.Session() as sess:
        res = sess.post(f"{url}/workflows/", json=data, headers=headers)
...
def get_all_attributes(cls: type, visited: set[type] | None = None) -> dict[str, Any]:
    """
    Recursively collect all attributes from a class and its superclasses.

    Parameters
    ----------
    cls
        The class to find attributes from
    ...
    for base in cls.__bases__:
        attributes |= get_all_attributes(base, visited)
