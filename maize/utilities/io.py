...
from maize.utilities.utilities import PathEncoder
...
class Config:
    ...
    telemetry_url
        URL for the maize telemetry server
    telemetry_key
        API key for the maize telemetry server
    ...
    def update(self, file: Path) -> None:
        ...
        for key, item in data.items():
            match key:
                ...
                case "telemetry_url":
                    self.telemetry_url = item
                case "telemetry_key":
                    self.telemetry_key = item
                case _:
...
def create_default_parser(help: bool = True) -> argparse.ArgumentParser:
    ...
    conf.add_argument(
        ...
        action="store_true",
        default=False,
        help="Provides all workflow inputs and outputs in JSON format",
    )
    return parser
...
def write_input(path: Path, data: dict[str, Any]) -> None:
    ...
    with path.open("w") as file:
        suffix = path.suffix.lower()[1:]
        if suffix == "json":
            file.write(json.dumps(data, indent=4, cls=PathEncoder))
        elif suffix in ("yaml", "yml"):
