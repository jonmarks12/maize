...
from maize.utilities.execution import JobResourceConfig, check_executable, _simple_run
...
from maize.utilities.utilities import timeout
...
def node_batch_jobs():
    class TestNode(Node):
        ...
        def run(self) -> None:
            n_jobs = self.inp.receive()
            self.run_multi(
                ["sleep 60"] * n_jobs,
                n_jobs=n_jobs,
                verbose=True,
                batch_options=JobResourceConfig(walltime="05:00:00"),
            )
...
def test_auto_job_cancel(tmp_path, node_batch_jobs):
    def n_queued() -> int:
        res = _simple_run("squeue --me")
        return len(res.stdout.decode().splitlines()[1:])

    flow = Workflow()
    data = flow.add(LoadData[int], parameters={"data": 2})
    out = flow.add(node_batch_jobs)
    flow.connect(data.out, out.inp)
    already_queued = n_queued()
    timeout(10)(flow.execute)()
    assert n_queued() == already_queued
