...
from maize.core.interface import Input, Port, Output, Parameter
...
def graph_dict_missing_field(graph_dict, request):
    ...
    graph[request.param][0].pop("name")
    return graph
...
class Test_Graph_init:
    ...
    def test_two_node_init(self, two_node_graph):
        ...
        assert len(two_node_graph.nodes["a"].parameters) == 10
...
class Test_Graph_build:
    ...
    def test_check_duplicate_param(self):
        g = Workflow()

        class A(Node):
            work_dir: Parameter[str] = Parameter(default="foo")
            out: Output[str] = Output()

            def run(self) -> None:
                self.out.send(self.work_dir.value)
