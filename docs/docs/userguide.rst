...
Job submission
""""""""""""""
The :meth:`~maize.core.node.Node.run_command` method is execution-environment agnostic, this means that a command can be run either locally or using a batch processing system such as `SLURM <https://slurm.schedmd.com>`_. To enable batch execution, the node author must pass the ``prefer_batch=True`` flag to :meth:`~maize.core.node.Node.run_command` (or just use :meth:`~maize.core.node.Node.run_multi`, where it is set to ``True`` by default) to signal a preference for batch execution when available, and the user must use the builtin parameter ``batch_options`` to specify the desired job resources.
...
.. code-block:: python
   ...
   class Example(Node):
       ...
       def run(self) -> None:
           data = self.inp.receive()
           # Runs on SLURM if Example.batch_options is set
           self.run_command(f"echo {data}", prefer_batch=True)
           self.out.send(data)

Global batch system settings, such as the type of system or default queue and walltime are handled in the maize configuration (see `configuring workflows <#config-workflow>`_) using :class:`~maize.utilities.execution.ResourceManagerConfig`, for example:
...
Setting the ``batch_options`` parameter can be done using a :class:`~maize.utilities.execution.JobResourceConfig` instance:

.. code-block:: python

   example.batch_options.set(JobResourceConfig(processes=2, custom_attributes={"gres": "gpu:1"}))

With the global settings above, this node will submit a *subjob* equivalent to this SLURM jobscript:
...
It is also possible to supply these options in a YAML workflow by using the custom python object syntax:
...
When using :meth:`~maize.core.node.Node.run_multi`, ``n_jobs`` refers to the maximum number of jobs to submit at once. The queuing system will be kept saturated with this number of jobs until they have completed.
